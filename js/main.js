(function($, undefined) {
	var	GOOGLE_API_KEY = 'AIzaSyC4JcblO8as1oi-1-erhWbxlLMJYBFbeNs',
		...
		locationCenters = {
			...
			'Eaton, OH': [39.7475, -84.633889],
			'Greenville, OH': [40.1025, -84.6281],
			'New Castle, IN': [39.9236, -85.3650],
			'Rushville, IN': [39.6144, -85.4486],
			'Portland, IN': [40.4339, -84.9800],
			'Columbus, IN': [39.2140, -85.9111],
			'Lima, OH': [40.7408, -84.1150],
			'Muncie, IN': [40.19434785396563, -85.38642883300781],
			'Greenfield': [39.785850963570304, -85.76957702636719],
			'Huntington, IN': [40.8819, -85.4956],
			'Bloomington, IN': [39.165325, -86.526386],
			'Wabash, IN': [40.8008, -85.8272],
			'Seymour, IN': [38.95922, -85.890255],
			'Batesville, IN': [39.2983, -85.2214]
		},
		placeTypes = {
			'accounting': {
				icon: 'lifestyle',
	...
	function search() {
		...
		$.each(locationCenters, function(location, coord) {
			...
			if (bounds === undefined || bounds.contains(center)) {	
				if (location === 'map-center'){searchRadius = 10000} else {searchRadius = 50000}
				var request = {
					location: center,
					radius: searchRadius,
					keyword: query
